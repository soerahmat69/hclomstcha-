...
module.exports = {
  ...
  addOrder: async (req, res) => {
    ...
    if(check[0].length > 0) {
      ...
      for (const letter of order_id) {
        const results = await pool.promise().query(
          `
        UPDATE character_order SET ? WHERE order_id = ?
            `,
          [data, letter]
        );
      }
     ...
     return  res.status(200).send({
      ...
      return  res.status(500).send({ 
        success: false,
        message: "data anda tidak lengkap, harap mengisikan data diri!"})
}
  },
  ...
  getReqOrderAcc: async (req, res, next) => {
    const results = await pool.promise().query(
      `
      SELECT  user.user_id,bukti_payment,character_order.order_id,user.username,character_anime.chara_name,character_order.tgl_rental FROM character_order 
